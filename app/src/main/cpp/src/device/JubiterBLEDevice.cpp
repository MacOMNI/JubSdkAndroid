...
JubiterBLEDevice::JubiterBLEDevice():
    _param{nullptr, BLE_ReadCallBack, BLE_ScanCallBack, BLE_StopScanCallback, BLE_DiscCallBack},
    _handle(0),
    _bConnected(false) {
...
JUB_RV JubiterBLEDevice::SendData(IN JUB_BYTE_CPTR sendData, IN JUB_ULONG ulSendLen,
    ...
    fido.clear();
    DEBUG_LOG(" send data: %s\n", jub::ByteArray2String(sendData, ulSendLen).c_str());
...
unsigned int JubiterBLEDevice::CancelConnect(unsigned char* devName, unsigned char* bBLEUUID) {
...
void JubiterBLEDevice::BLE_StopScanCallback() {
//    if (!(IS_BLE_MODE)) {
//        return;
//    }
    ...
    auto bleDevice = Singleton<jub::JubiterBLEDevice>::GetInstance();
    if (bleDevice) {
        if (bleDevice->outerParams.stopScanCallback) {
            bleDevice->outerParams.stopScanCallback();
        }
    }
