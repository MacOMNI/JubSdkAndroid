...
JUB_RV JUB_connectDevice(JUB_BYTE_PTR devName,
    ...
    JUB_RV rv = bleDevice->Connect(devName, bBLEUUID, connectType, pdevHandle, timeout);
...
//    LOG_INF("JUB_connectDevice rv: %hu", *pDeviceID);
    ...
    else if (jub::JubiterBLDImpl::ConfirmType(devName)) {
...
JUB_RV JUB_cancelConnect(JUB_BYTE_PTR devName, JUB_BYTE_PTR bBLEUUID) {
    ...
    JUB_VERIFY_RV(bleDevice->CancelConnect(devName, bBLEUUID));
