...
JUB_RV JubiterBLDImpl::SelectAppletTRX() {
...
//MISC functions
JUB_RV JubiterBLDImpl::SetCoinTypeTRX() {

    return _SetCoinType((JUB_BYTE)JUB_ENUM_COINTYPE_MISC::COINTRX);
}


JUB_RV JubiterBLDImpl::GetAddressTRX(const std::string& path, const JUB_UINT16 tag, std::string& address) {
...
JUB_RV JubiterBLDImpl::GetHDNodeTRX(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
...
JUB_RV JubiterBLDImpl::SignTXTRX(const std::vector<JUB_BYTE>& vPath,
...
JUB_RV JubiterBLDImpl::SetTRC20Token(const std::string& tokenName,
