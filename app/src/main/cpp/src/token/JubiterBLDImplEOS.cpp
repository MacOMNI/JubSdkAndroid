...
JUB_RV JubiterBLDImpl::GetHDNodeEOS(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
...
JUB_RV JubiterBLDImpl::SignTXEOS(const TW::EOS::Type& type,
    ...
    try {
        ...
        TW::EOS::encodeCollection(tx.contextFreeActions, vContextFreeActions);
        ...
        TW::EOS::encodeCollection(tx.actions, vActions);
        ...
        TW::EOS::encodeCollection(tx.transactionExtensions, vTransactionExtensions);
