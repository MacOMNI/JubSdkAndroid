...
JUB_RV JubiterBLDBioImpl::VerifyFingerprint(OUT JUB_ULONG &retry) {
    ...
    APDU apdu(0x00, 0x20, (JUB_ULONG)JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_FPGT, 0x00, 0x00);
    JUB_UINT16 ret = 0;
    JUB_BYTE retData[1024] = {0,};
    JUB_ULONG ulRetDataLen = sizeof(retData)/sizeof(JUB_BYTE);
    JUB_VERIFY_RV(_SendApdu(&apdu, ret, retData, &ulRetDataLen));
...
JUB_RV JubiterBLDBioImpl::EnrollFingerprint(IN JUB_UINT16 fpTimeout,
    ...
    size_t vSize = tlv_buf::decodeTV(0x8210, abcd::DataChunk(retData, retData + ulRetDataLen), v);
    if (!(v.size() == vSize
          &&     3 == vSize)
        ) {
        return JUBR_ERROR;
